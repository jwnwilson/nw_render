...
void VeiwSys::translate(const Vector3D& trans)
{
	eyePos=eyePos+trans;
...
void VeiwSys::setScreen(const Vector3D& BL,const Vector3D& BR,const Vector3D& TR,const Vector3D& TL)
{
	screenBotLef=BL;
	screenBotRig=BR;
	screenTopLef=TL;
...
void VeiwSys::initialise(FrameBuffer* fb)
{
	if(fb!=NULL)
	{
		screen=fb;
...
float VeiwSys::findRot(const Vector2D& v1,const Vector2D& v2)
{
	float ang;
...
void VeiwSys::setTarget(const Vector3D& target)
{
...
Vector3D VeiwSys::getRayVec(int x,int y)
{
	Vector3D rayVec;
...
Ray VeiwSys::getRay(int x,int y)
{
	Ray ray1;
